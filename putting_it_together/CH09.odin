...
CH09 :: proc(c: rt.Canvas) {
    ...
    floor := rt.plane(&sb, floor_mat);

    wall_mat := rt.material(color=rt.color(.07, .07, .07), ambient=.4, specular=0);
    wall1 := rt.plane(&sb, m.translation(0, 0, 0.05) * m.rotation_x(PI / 2), wall_mat);
    wall_mat.color = rt.color(.2, .3, .8);
    wall_mat.ambient = .4;
    wall_mat.specular = .2;
    wall2 := rt.plane(&sb, m.translation(-1, 0, -0.5) * m.rotation_y(PI / 3.2) * m.rotation_z(PI / 2), wall_mat);
