...
CH13_3 :: proc(c: rt.Canvas) {
    c := c;
    fmt.println("Putting it together for chapter 13.3");
    ...
    ok := rt.ppm_write_to_file("images/putting_it_together_ch13.3.ppm", ppm);
    if !ok {
        panic("Failed to write ppm file...");
    }
}

CH13_4 :: proc(c: rt.Canvas) {
    c := c;
    fmt.println("Putting it together for chapter 13.4");
    ...
    cone1_mat := rt.material(color=rt.color(0.2, 0.9, 0.2), reflective=0.3, transparency=.3, diffuse=.2, refractive_index=1);

    cone1_tf := m.translation(-1.2, 0.9, 0.8) * m.rotation_x(-PI / 6) * m.scaling(0.5, 0.5, 0.5);
    cone1 := rt.cone(cone1_tf, cone1_mat);
    cone1.minimum = -1;
    cone1.maximum = 1;
    cone1.closed = true;
    ...
    shapes := []^rt.Shape { &floor, &cone1, &sphere1, &cube1, &cube2, &cube3 };
    ...
    ok := rt.ppm_write_to_file("images/putting_it_together_ch13.4.ppm", ppm);
    if !ok {
        panic("Failed to write ppm file...");
    }
}
