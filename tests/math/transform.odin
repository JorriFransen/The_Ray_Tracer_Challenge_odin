...
P_Mul_Translation :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.translate(p, 5, -3, 2) == expected);
}
...
V_Mul_Translation :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.translate(v, 5, -3, 2) == v);
}
...
P_Mul_Scale :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.scale(p, 2, 3, 4) == expected);
}
...
V_Mul_Scale :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.scale(v, 2, 3, 4) == expected);
}
...
V_Mul_Inv_Scale :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.scale(v, 1.0 / 2, 1.0 / 3, 1.0 / 4) == expected);
...
Reflection_Is_Neg_Scale :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.scale(p, -1, 1, 1) == expected);
}
...
Rot_Around_X :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(half_result_1, expected1));
    r.expect(t, rm.eq(half_result_2, expected1));
    r.expect(t, rm.eq(rm.rotate_x(p, PI / 4), expected1));

    r.expect(t, rm.eq(full_result_1, expected2));
    r.expect(t, rm.eq(full_result_2, expected2));
    r.expect(t, rm.eq(rm.rotate_x(p, PI / 2), expected2));
}
...
Rot_Around_X_Inv :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(rm.rotate_x(p, -PI / 4), expected));

    {
        inv := rm.matrix_inverse(rm.rotate_x(rm.matrix4_identity, PI / 4));
        r.expect(t, rm.eq(rm.mul(inv, p), expected));
    }
}
...
Rot_Around_Y :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(half_result_1, expected1));
    r.expect(t, rm.eq(half_result_2, expected1));
    r.expect(t, rm.eq(rm.rotate_y(p, PI / 4), expected1));

    r.expect(t, rm.eq(full_result_1, expected2));
    r.expect(t, rm.eq(full_result_2, expected2));
    r.expect(t, rm.eq(rm.rotate_y(p, PI / 2), expected2));
}
...
Rot_Around_Y_Inv :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.rotate_y(p, -PI / 4), expected));
...
Rot_Around_Z :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(half_result_1, expected1));
    r.expect(t, rm.eq(half_result_2, expected1));
    r.expect(t, rm.eq(rm.rotate_z(p, PI / 4), expected1));

    r.expect(t, rm.eq(full_result_1, expected2));
    r.expect(t, rm.eq(full_result_2, expected2));
    r.expect(t, rm.eq(rm.rotate_z(p, PI / 2), expected2));
}
...
Rot_Around_Z_Inv :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.rotate_z(p, -PI / 4), expected));
...
Shear_XY :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.shear(p, 1, 0, 0, 0, 0, 0), expected));
}
...
Shear_XZ :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.shear(p, 0, 1, 0, 0, 0, 0), expected));
}
...
Shear_YX :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.shear(p, 0, 0, 1, 0, 0, 0), expected));
}
...
Shear_YZ :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.shear(p, 0, 0, 0, 1, 0, 0), expected));
}
...
Shear_ZX :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.shear(p, 0, 0, 0, 0, 1, 0), expected));
}
...
Shear_ZY :: proc(t: ^testing.T) {
    ...
    r.expect(t, rm.eq(result1, expected));
    r.expect(t, rm.eq(result2, expected));
    r.expect(t, rm.eq(rm.shear(p, 0, 0, 0, 0, 0, 1), expected));
}
...
Sequenced :: proc(t: ^testing.T) {
    ...
    {
        p2 := rm.mul(A, p);
        r.expect(t, rm.eq(p2, rm.point(1, -1, 0)))

        p3 := rm.mul(B, p2);
        r.expect(t, rm.eq(p3, rm.point(5, -5, 0)));

        p4 := rm.mul(C, p3);
        r.expect(t, rm.eq(p4, rm.point(15, 0, 7)));
    }
...
Chained :: proc(t: ^testing.T) {
...

    {
        ...
        r.expect(t, rm.eq(result1, expected));
        r.expect(t, rm.eq(result11, expected))

        r.expect(t, rm.eq(result2, expected));
        r.expect(t, rm.eq(result22, expected))
    }

    {
        T1 := rm.translate(rm.scale(rm.rotation_x(PI / 2), 5, 5, 5), 10, 5, 7);

        r.expect(t, rm.eq(T, T1));
